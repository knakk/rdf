...
func BenchmarkDecodeTTL(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		dec := NewTripleDecoder(bytes.NewBufferString(input), FormatTTL, IRI{str: ""})
...
func TestEncodingTTL(t *testing.T) {
	for i, ex := range ttlBenchInputs {
		dec := NewTripleDecoder(bytes.NewBufferString(ex), FormatTTL, IRI{str: ""})
...
func TestTTL(t *testing.T) {
	for _, test := range ttlTestSuite {
		dec := NewTripleDecoder(bytes.NewBufferString(test.input), FormatTTL, IRI{str: ""})
		triples, err := dec.DecodeAll()
		if err != nil {
			if test.errWant == "" {
